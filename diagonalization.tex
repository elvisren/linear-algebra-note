...
If a square matrix is diagonalizable, it is similar to a diagonal matrix. So $D = \inverse{Q} A Q$ where each column of $Q$ is a eigenvector ($Q = \coordinate{I_V}_v^e$).
...
Theorem~\ref{eigenvector_sets_are_linearly_independent} shows that if $T$ has $n$ distinct eigenvalue, then $T$ must be diagonalizable.
...
It is easy to see that $A$ is invertible $\Leftrightarrow$ $0$ is not an eigenvalue.
...
\begin{proof}
    ...
    \begin{equation*}
        T(\beta_j) = T(T^{j-1}(v)) = T^{j}(v) = \begin{pmatrix}
            0 \\
            \vdots \\
            1_{j} \\
            \vdots \\
...
\end{theorem}
\begin{proof}
    For any vector $v$, find its $T$-cyclic subspace and generate its polynomial. Use Theorem~\ref{split_of_invariant_subspace} and Theorem~\ref{char_poly_of_invariant_subspace} to prove.
\end{proof}
