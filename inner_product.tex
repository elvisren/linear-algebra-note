...
The answer is yes. It obviously puts requirement on both the $V$ and $T$. It turns out that the $T$ must be normal (in $C$) or self-adjoint (in $R$). So we prefer complex normal and real symmetric matrices. The operators here are complex normal or real symmetric.
...
\begin{enumerate}
    ...
    \item $T$ is positive semidefinite if and only if $A = B^*B$ for some $B$
\end{enumerate}


\begin{theorem}\label{tt_positive_semidefinite}
    Let $T:V \rightarrow W$ be a linear transformation, $TT^*$ and $T^*T$ are positive semidefinite.
\end{theorem}
...
\begin{definition}[Adjoint of Linear Transformation]
    ...
    \begin{equation}
        \innerproduct{T(x)}{y}_W = \innerproduct{x}{T^*(y)}_V
    \end{equation}
\end{definition}

The adjoint of linear transformation has similar property:
\begin{enumerate}
    \item $\innerproduct{x}{T(y)}_V = \innerproduct{T(x)}{y}_W$
...
\begin{proof}
    ...
    So $\set{w_i}_{i \leq r}$ are orthonormal, and we can extent it to $\set{w_m}$.
\end{proof}
...
\begin{theorem}[Polar Decomposition]
    ...
    A similar representation in $C$ is that for $x \in C$, $\displaystyle x = \frac{x}{\norm{x}} \cdot \norm{x}$. The first part is unitary and the second part is positive.
\end{theorem}
\begin{proof}
