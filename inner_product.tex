...
\begin{example}
    ...
    \end{equation}
     ...
     This is an expression of $\norm{f - g}^2$, so we would like to find a $g$ that $g \perp f$. We could project $f$ on $U$, which is to project $f$ on basis of $U$ and add up the projections.
\end{example}
...
\begin{theorem}\label{normal_orthonormal_with_complex_space}
    For finite-dimension inner product space $V$ on $F=\complexset$, $T$ is normal if and only if there exists orthonormal basis that are eigenvectors of $T$.
\end{theorem}
\begin{proof}
...
\theoref{normal_orthonormal_with_complex_space} did not work for $F=\realset$ because polynomials do not always split. So we need stronger restriction to make it split in $\realset$, which is self-adjoint.
...
\begin{enumerate}
    ...
    \item If $V$ is on $F=\realset$, $T$ still splits
\end{enumerate}
\begin{proof}
    \begin{equation*}
    ...
    For second property, take $F=\complexset$ and let $T$ splits in $\complexset$. But all eigenvalues are real, so $T$ splits in $\realset$.
\end{proof}

\begin{theorem}
    For finite-dimension inner product space $V$ on $F=\realset$, $T$ is self-adjoint if and only if there exists orthonormal basis that are eigenvectors of $T$.
\end{theorem}
\begin{proof}
...
\begin{theorem}
    ...
    \begin{enumerate}
        ...
        \item $T$ is self-adjoint if and only if $T$ is normal and $\lambda \in \realset$
    \end{enumerate}    
\end{theorem}
\begin{proof}
